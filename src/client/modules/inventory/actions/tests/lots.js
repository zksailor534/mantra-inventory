...
import actions from '../lots';

describe('actions.lots', () => {
  describe('create', () => {
    it('should reject if recordId is not there', () => {
      const LocalState = { set: spy() };
      actions.create({ LocalState }, null, 'product');
    ...
    it('should reject if productName is not there', () => {
      const LocalState = { set: spy() };
      actions.create({ LocalState }, 'id', null);
      const args = LocalState.set.args[0];
    ...
    it('should clear older LocalState for SAVING_ERROR', () => {
      ...
      actions.create({ LocalState, Meteor, FlowRouter }, 'r', 'p');
      expect(LocalState.set.args[0]).to.deep.equal([ 'SAVING_ERROR', null ]);
    });

    it('should call Meteor.call to save the lot', () => {
    ...
    it('should redirect user to the lot', () => {
      const id = 'dsds';
      ...
      actions.create({ Meteor, LocalState, FlowRouter }, 'r', 'p');
      expect(FlowRouter.go.args[0][0]).to.be.equal(`/lot/${id}`);
    });
